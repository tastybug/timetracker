...
public class TrackingRecordToReportableMapperTest {
    ...
    public void mapRecords_throws_NPE_when_given_null_TrackingRecord_list() {
        // expect
        subject.mapRecords(null, new TrackingConfiguration(""));
    }

    @Test(expected = NullPointerException.class)
    public void mapRecords_throws_NPE_when_given_null_TrackingConfiguration() {
        // expect
        subject.mapRecords(new ArrayList<TrackingRecord>(), null);
    }

    @Test
    public void mapRecords_returns_empty_result_list_for_empty_tracking_record_list() {
        // when
        List<ReportableItem> reportables = subject.mapRecords(new ArrayList<TrackingRecord>(), new TrackingConfiguration(""));
    ...
    public void mapRecords_returns_mapped_instances() {
        ...
        // when
        List<ReportableItem> reportables = subject.mapRecords(trackingRecords, trackingConfiguration);
        ...
        assertEquals(reportables.get(0).getDuration(), trackingRecord1.toEffectiveDuration(trackingConfiguration).get());
        ...
        assertEquals(reportables.get(1).getDuration(), trackingRecord2.toEffectiveDuration(trackingConfiguration).get());
    }
}